module.exports = {
  ...
  deploy: {
    production: {
      ...
      'pre-deploy-local': '',
      'post-deploy':
        'rm -f .env.development && ln -sf /var/www/simple-shop-server/.env.production .env && pnpm install --frozen-lockfile && pnpm run build && pnpm run migration:run:prod && pm2 reload ecosystem.config.js --env production',
